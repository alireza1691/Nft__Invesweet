...
contract Proxy {
    ...
    bytes32 private constant ERC721Creator_SLOT =
    ...
    function _getAdmin() private view returns (address) {
        return StorageSlot.getAddress(ADMIN_SLOT).value;
    }

    function _setAdmin(address _admin) private {
        require(_admin != address(0), "admin = zero address");
        StorageSlot.getAddress(ADMIN_SLOT).value = _admin;
    }

    function _getImplementation() private view returns (address) {
        return StorageSlot.getAddress(ERC721Creator_SLOT).value;
    }

    function _setImplementation(address _implementation) private {
        require(_implementation.code.length > 0, "implementation is not contract");
        StorageSlot.getAddress(ERC721Creator_SLOT).value = _implementation;
    }
...
//         StorageSlot.getAddress(slot).value = _addr;
//     }
// }